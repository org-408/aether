...
use crate::actor::supervisor::supervisor_strategy::{
    log_failure, Supervisor, SupervisorHandle, SupervisorStrategy,
};
...
impl SupervisorStrategy for ExponentialBackoffStrategy {
    async fn handle_child_failure(
        ...
        let backoff = rs.failure_count().await as u64
            * self.initial_backoff.map(|v| v.as_nanos()).unwrap_or(0) as u64;
