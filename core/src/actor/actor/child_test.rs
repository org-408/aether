...
pub mod tests {
    ...
    impl Message for GetChildCountMessage2 {
        fn eq_message(&self, other: &dyn Message) -> bool {
            ...
            match other_msg {
                Some(other_msg) => {
                    self.reply_directly == other_msg.reply_directly
                        && self.reply_after_stop == other_msg.reply_after_stop
                }
                None => false,
            }
        }
    ...
    async fn test_actor_can_stop_children() {
        ...
        let a = root_context
            .spawn(
                Props::from_async_actor_producer(|_| async {
                    CreateChildThenStopActor { reply_to: None }
                })
                .await,
            )
            .await;
        ...
        assert_eq!(
            0,
            r2.to_typed::<GetChildCountResponse>().unwrap().child_count
        );
    }
            ...
            .spawn(
                Props::from_async_actor_receiver(move |mut ctx| {
                    ...
                    async move {
                        ...
                        if let Some(AutoReceiveMessage::PostStart) =
                            msg.to_typed::<AutoReceiveMessage>()
                        {
                            ctx.watch(&cloned_child).await;
                            cloned_ab.wait().await;
                        }
                        if let Some(AutoReceiveMessage::Terminated(ti)) =
                            msg.to_typed::<AutoReceiveMessage>()
                        {
                            ...
                            if ti.who.unwrap() == cloned_child.inner_pid
                                && ac
                                    ...
                                    .await
                            {
                                ctx.send(cloned_future.get_pid().await, MessageHandle::new(true))
                                    .await;
                            }
                        }
                        Ok(())
