...
pub struct SenderMiddleware(
    Arc<dyn Fn(SenderMiddlewareChain) -> SenderMiddlewareChain + Send + Sync + 'static>,
);
...
impl std::hash::Hash for SenderMiddleware {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        (self.0.as_ref() as *const dyn Fn(SenderMiddlewareChain) -> SenderMiddlewareChain)
            .hash(state);
    }
}
...
impl SenderMiddleware {
    pub fn new(
        f: impl Fn(SenderMiddlewareChain) -> SenderMiddlewareChain + Send + Sync + 'static,
    ) -> Self {
