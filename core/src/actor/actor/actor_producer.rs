...
pub struct ActorProducer(
    Arc<dyn Fn(ContextHandle) -> BoxFuture<'static, ActorHandle> + Send + Sync + 'static>,
);
...
impl ActorProducer {
    ...
    {
        Self(Arc::new(move |ch| {
            Box::pin(f(ch)) as BoxFuture<'static, ActorHandle>
        }))
    }
...
impl std::hash::Hash for ActorProducer {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        (self.0.as_ref() as *const dyn Fn(ContextHandle) -> BoxFuture<'static, ActorHandle>)
            .hash(state);
    }
}
