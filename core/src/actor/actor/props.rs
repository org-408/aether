...
static DEFAULT_MAILBOX_PRODUCER: Lazy<MailboxProducer> =
    Lazy::new(|| unbounded_mailbox_creator_with_opts(vec![]));
...
static DEFAULT_SPAWNER: Lazy<Spawner> = Lazy::new(|| {
    Spawner::new(
        |actor_system: ActorSystem,
         name: String,
         props: Props,
         parent_context: SpawnerContextHandle| async move {
            tracing::debug!("Spawn actor: {}", name);
            let mut ctx = ActorContext::new(
                actor_system.clone(),
                props.clone(),
                parent_context.get_self_opt().await,
            )
            .await;
            let mut mb = props.produce_mailbox().await;

            let dp = DispatcherHandle::new_arc(
                actor_system.get_config().await.system_dispatcher.clone(),
            );
            ...
            mb.register_handlers(Some(mi.clone()), Some(dp.clone()))
                .await;
            ...
            mb.post_system_message(MessageHandle::new(SystemMessage::Start))
                .await;
...
impl Props {
    ...
    pub fn with_context_decorators(
        decorators: impl IntoIterator<Item = ContextDecorator> + Send + Sync,
    ) -> PropsOption {
    ...
    pub fn with_sender_middlewares(
        middlewares: impl IntoIterator<Item = SenderMiddleware> + Send + Sync,
    ) -> PropsOption {
        let middlewares = middlewares.into_iter().collect::<Vec<_>>();
        PropsOption::new(move |props: &mut Props| {
            ...
            props.sender_middleware_chain = make_sender_middleware_chain(
                ...
                SenderMiddlewareChain::new(|sch, target, me| async move {
                    target
                        .send_user_message(
                            sch.get_actor_system().await.clone(),
                            MessageHandle::new(me),
                        )
                        .await
                }),
            );
    ...
    pub(crate) async fn spawn(
        ...
        self.get_spawner()
            .run(actor_system, name, self, parent_context)
            .await
    }
