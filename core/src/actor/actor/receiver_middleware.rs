...
#[derive(Clone)]
pub struct ReceiverMiddleware(
    Arc<dyn Fn(ReceiverMiddlewareChain) -> ReceiverMiddlewareChain + Send + Sync + 'static>,
);
...
impl ReceiverMiddleware {
    pub fn new(
        f: impl Fn(ReceiverMiddlewareChain) -> ReceiverMiddlewareChain + Send + Sync + 'static,
    ) -> Self {
...
impl std::hash::Hash for ReceiverMiddleware {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        (self.0.as_ref() as *const dyn Fn(ReceiverMiddlewareChain) -> ReceiverMiddlewareChain)
            .hash(state);
    }
}
