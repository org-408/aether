...
pub struct ContextDecorator(
    Arc<dyn Fn(ContextDecoratorChain) -> ContextDecoratorChain + Send + Sync + 'static>,
);
...
impl ContextDecorator {
    pub fn new(
        f: impl Fn(ContextDecoratorChain) -> ContextDecoratorChain + Send + Sync + 'static,
    ) -> Self {
...
impl std::hash::Hash for ContextDecorator {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        (self.0.as_ref() as *const dyn Fn(ContextDecoratorChain) -> ContextDecoratorChain)
            .hash(state);
    }
}
