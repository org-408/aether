...
use crate::actor::message::{
    Message, MessageHandle, ReadonlyMessageHeadersHandle, TypedMessageEnvelope,
};
...
pub trait TypedSpawnerContext<M: Message>:
    TypedInfoPart<M> + TypedSpawnerPart + Send + Sync + 'static
{
}
...
pub trait TypedMessagePart<M: Message>: Debug + Send + Sync + 'static {
    ...
    async fn get_message_handle(&self) -> MessageHandle {
        self.get_message_handle_opt()
            .await
            .expect("message handle not found")
    }
...
pub trait TypedStopperPart<M: Message>: Debug + Send + Sync + 'static {
    ...
    async fn stop_future(&mut self, pid: &TypedExtendedPid<M>) -> ActorFuture {
        self.stop_future_with_timeout(pid, Duration::from_secs(10))
            .await
    }
