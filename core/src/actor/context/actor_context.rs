...
use crate::actor::context::{
    BasePart, Context, ExtensionContext, ExtensionPart, InfoPart, MessagePart, ReceiverContext,
...
use crate::actor::message::{
    unwrap_envelope_header, unwrap_envelope_message, unwrap_envelope_sender, wrap_envelope,
    MessageEnvelope,
};
...
impl ActorContext {
    ...
    pub(crate) async fn ensure_extras(&mut self) -> ActorContextExtras {
        if self.get_extras().await.is_none() {
            ...
            self.set_extras(Some(ActorContextExtras::new(ctxd).await))
                .await;
        }
    ...
    pub async fn send_user_message(&self, pid: ExtendedPid, message_handle: MessageHandle) {
        match self.get_sender_middleware_chain().await {
            Some(chain) => {
                ...
                chain
                    .run(context, pid, MessageEnvelope::new(message_handle))
                    .await;
            }
            _ => {
                pid.send_user_message(self.get_actor_system().await, message_handle)
                    .await;
            }
        }
    }
    ...
    async fn handle_start(&mut self) -> Result<(), ActorError> {
        self.invoke_user_message(MessageHandle::new(AutoReceiveMessage::PostStart))
            .await?;
        Ok(())
    }
...
impl BasePart for ActorContext {
    ...
    async fn forward(&self, pid: &ExtendedPid) {
        ...
        if let Some(message_or_envelope) = &*mg {
            if let Some(sm) = message_or_envelope.to_typed::<SystemMessage>() {
                ...
                pid.send_user_message(self.get_actor_system().await, message_or_envelope.clone())
                    .await;
            }
        }
    }
    ...
    async fn reenter_after(
        &self,
        future: crate::actor::dispatch::future::ActorFuture,
        continuer: Continuer,
    ) {
...
impl SenderPart for ActorContext {
    ...
    async fn request(&mut self, pid: ExtendedPid, message_handle: MessageHandle) {
        let env =
            MessageEnvelope::new(message_handle).with_sender(self.get_self_opt().await.unwrap());
        let message_handle = MessageHandle::new(env);
        self.send_user_message(pid, message_handle).await;
    }

    async fn request_with_custom_sender(
        &mut self,
        pid: ExtendedPid,
    ...
    async fn request_future(
        ...
        let future_process =
            ActorFutureProcess::new(self.get_actor_system().await, timeout.clone()).await;
...
impl ReceiverPart for ActorContext {
    async fn receive(&mut self, envelope: MessageEnvelope) -> Result<(), ActorError> {
        self.set_message_or_envelope(MessageHandle::new(envelope))
            .await;
        let result = self.default_receive().await;
        self.reset_message_or_envelope().await;
        result
    ...
    async fn spawn_named(&mut self, props: Props, id: &str) -> Result<ExtendedPid, SpawnError> {
        ...
        let result = match self.get_props().await.get_spawn_middleware_chain() {
            Some(chain) => {
                ...
                chain
                    .run(self.get_actor_system().await, &id, props, sch)
                    .await
            }
            _ => {
                let sch = SpawnerContextHandle::new(self.clone());
...
impl StopperPart for ActorContext {
    async fn stop(&mut self, pid: &ExtendedPid) {
        ...
        .await;
        let inner_mg = self.inner.lock().await;
        pid.ref_process(inner_mg.actor_system.clone())
            .await
            .stop(&pid)
            .await;
    }
...
impl MessageInvoker for ActorContext {
    ...
    async fn invoke_user_message(
        ...
        if receive_timeout.unwrap_or_else(|| Duration::from_millis(0)) > Duration::from_millis(0) {
            influence_timeout = !message_handle
                .to_typed::<NotInfluenceReceiveTimeoutHandle>()
                .is_some();
...
impl Supervisor for ActorContext {
    ...
    async fn escalate_failure(&self, reason: ErrorReason, message_handle: MessageHandle) {
        let self_pid = self
            .get_self_opt()
            .await
            .expect("Failed to retrieve self_pid");
        if self
            .get_actor_system()
            .await
            .get_config()
        ...
        self.get_self_opt()
            ...
            .send_system_message(
                self.get_actor_system().await,
                MessageHandle::new(failure.clone()),
            )
            .await;
