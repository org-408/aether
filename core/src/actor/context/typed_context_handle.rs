use crate::actor::actor::{
    ActorError, ActorHandle, Continuer, ExtendedPid, SpawnError, TypedExtendedPid, TypedProps,
};
use crate::actor::actor_system::ActorSystem;
use crate::actor::context::{
    BasePart, ContextHandle, ExtensionContext, ExtensionPart, InfoPart, MessagePart, ReceiverPart,
...
use crate::actor::typed_context::{
    TypedContext, TypedInfoPart, TypedMessagePart, TypedReceiverContext, TypedReceiverPart,
...
impl<M: Message> TypedSenderPart<M> for TypedContextHandle<M> {
    ...
    async fn send<A: Message>(&mut self, pid: TypedExtendedPid<A>, message: A) {
        self.underlying
            .send(pid.into(), MessageHandle::new(message))
            .await
    }

    async fn request<A: Message>(&mut self, pid: TypedExtendedPid<A>, message: A) {
        self.underlying
            .request(pid.into(), MessageHandle::new(message))
            .await
    }
    ...
    async fn request_future<A: Message>(
        &self,
        pid: TypedExtendedPid<A>,
        message: A,
        timeout: Duration,
...
impl<M: Message> TypedSpawnerPart for TypedContextHandle<M> {
    ...
    async fn spawn_prefix<A: Message + Clone>(
        &mut self,
        props: TypedProps<A>,
        prefix: &str,
    ) -> TypedExtendedPid<A> {
        self.underlying
            .spawn_prefix(props.into(), prefix)
            .await
            .into()
    }
