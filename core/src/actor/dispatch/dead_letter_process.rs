            ...
            .subscribe(move |msg| {
                ...
                async move {
                    if let Some(dead_letter) = cloned_msg.to_typed::<DeadLetterEvent>() {
                        ...
                        if let Some(is_ignore_dead_letter) =
                            dead_letter.message_handle.to_typed::<IgnoreDeadLetterLogging>()
                        {
            ...
            .subscribe(move |msg| {
                ...
                async move {
                    if let Some(dle) = cloned_msg.to_typed::<DeadLetterEvent>() {
                        if let Some(SystemMessage::Watch(watch)) =
                            dle.message_handle.to_typed::<SystemMessage>()
                        {
...
impl Process for DeadLetterProcess {
    async fn send_user_message(&self, pid: Option<&ExtendedPid>, message_handle: MessageHandle) {
        tracing::debug!(
            "DeadLetterProcess: send_user_message: msg = {:?}",
            message_handle
        );
    ...
    async fn send_system_message(&self, pid: &ExtendedPid, message_handle: MessageHandle) {
        self.actor_system
            ...
            .await;
        tracing::debug!(
            "DeadLetterProcess: send_system_message: msg = {:?}",
            message_handle
        );
    }

    async fn stop(&self, pid: &ExtendedPid) {
        self.send_system_message(pid, MessageHandle::new(SystemMessage::Stop))
            .await
    }
