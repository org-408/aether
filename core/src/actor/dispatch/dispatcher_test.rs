...
mod test {
    ...
    use aether_utils::collections::MpscUnboundedChannelQueue;
    ...
    impl MessageInvoker for TestMessageInvoker {
        ...
        async fn escalate_failure(&mut self, reason: ErrorReason, _: MessageHandle) {
            ...
            self.received
                .lock()
                .await
                .push(ReceivedMessage::Failure(reason_msg));
        }
    }
    ...
    async fn test_mailbox_with_test_invoker() {
        ...
        mailbox
            .register_handlers(
                Some(invoker_handle.clone()),
                Some(dispatcher_handle.clone()),
            )
            .await;
