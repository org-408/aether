...
use aether_utils::collections::{QueueError, QueueReader, QueueWriter};
...
impl DefaultMailbox {
    ...
    pub(crate) async fn with_middlewares(
        self,
        middlewares: impl IntoIterator<Item = MailboxMiddlewareHandle>,
    ) -> Self {
        {
    ...
    async fn compare_exchange_scheduler_status(
        &self,
        current: bool,
        new: bool,
    ) -> Result<bool, bool> {
    ...
    async fn poll_system_mailbox(
        &self,
    ) -> Result<Option<MessageHandle>, QueueError<MessageHandle>> {
        let inner_mg = self.inner.lock().await;
    ...
    async fn schedule(&self) {
        if self
            .compare_exchange_scheduler_status(false, true)
            .await
            .is_ok()
        {
            let dispatcher = self
    ...
    async fn run(&self) {
        ...
        if self.get_dispatcher_opt().await.is_none()
            || self.get_message_invoker_opt().await.is_none()
        {
            return;
        }
    ...
    async fn process_messages(&self) {
        loop {
            ...
            if (system_messages_count > 0
                || (!self.is_suspended().await && user_messages_count > 0))
                && self
                    .compare_exchange_scheduler_status(false, true)
                    .await
                    .is_ok()
