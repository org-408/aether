...
impl ActorMetrics {
    pub fn new(
        meter_provider: Arc<MetricsProvider>,
    ) -> Result<Self, opentelemetry::metrics::MetricsError> {
    ...
    pub async fn record_actor_message_receive_duration_with_opts(
        &self,
        duration: f64,
        attributes: &[KeyValue],
    ) {
        let inner_mg = self.inner.lock().await;
        inner_mg
            .actor_message_receive_histogram
...
mod tests {
    ...
    async fn test_actor_metrics() {
        ...
        let metrics =
            ActorMetrics::new(Arc::new(meter_provider)).expect("メトリクスの初期化に失敗しました");
