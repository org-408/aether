...
use aether_core::actor::actor::{Actor, ActorError, ExtendedPid, Props};
use aether_core::actor::actor_system::ActorSystem;
use aether_core::actor::context::{
    ContextHandle, InfoPart, MessagePart, SenderPart, SpawnerPart,
};
...
impl RequestTaskBehavior {
    ...
    async fn request_more(&mut self) {
        ...
        self.actor_system
            ...
            .send(
                self.producer.clone(),
                MessageHandle::new(RequestMoreTask::new(50)),
            )
            .await;
    }
}
...
impl Producer {
    ...
    async fn mailbox_producer(&self, ctx: ContextHandle) -> MailboxProducer {
        unbounded_mailbox_creator_with_opts([MailboxMiddlewareHandle::new(
            RequestTaskBehavior::new(ctx.get_actor_system().await, 0, ctx.get_self().await),
        )])
    }
    ...
    async fn consumer_props(&self, ctx: ContextHandle) -> Props {
        Props::from_sync_actor_producer_with_opts(
            ...
            [Props::with_mailbox_producer(
                self.mailbox_producer(ctx.clone()).await,
            )],
        )
        .await
    }
...
impl Actor for Producer {
    async fn receive(&mut self, mut ctx: ContextHandle) -> Result<(), ActorError> {
        ...
        if let Some(request_more_work) =
            ctx.get_message_handle().await.to_typed::<RequestMoreTask>()
        {
            self.requested_tasks += request_more_work.items;
            self.produced_tasks = 0;
            ctx.send(ctx.get_self().await, MessageHandle::new(Produce))
                .await;
        }
        if ctx
            .get_message_handle()
...
async fn main() {
    ...
    let system = ActorSystem::new()
        .await
        .expect("Failed to create an actor system");
