...
use aether_core::actor::actor::{ActorError, TypedActor, TypedProps};
use aether_core::actor::actor_system::ActorSystem;
use aether_core::actor::context::TypedContextHandle;
use aether_core::actor::dispatch::unbounded_mpsc_mailbox_creator;
use aether_core::actor::message::Message;
use aether_core::actor::typed_context::{
    TypedMessagePart, TypedSenderPart, TypedSpawnerPart,
};
...
impl TypedActor<Hello> for TopActor {
    async fn post_start(
        &mut self,
        mut context_handle: TypedContextHandle<Hello>,
    ) -> Result<(), ActorError> {
        ...
        for _ in 1..10 {
            context_handle
                .send(pid.clone(), Hello("hello-2".to_string()))
                .await;
        }
        Ok(())
    }
...
async fn main() {
    ...
    let props = TypedProps::from_async_actor_producer_with_opts(
        ...
        [Props::with_mailbox_producer(
            unbounded_mpsc_mailbox_creator(),
        )],
    )
    .await;
