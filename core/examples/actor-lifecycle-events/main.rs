...
use aether_core::actor::actor::{Actor, ActorError, ErrorReason, Props};
use aether_core::actor::actor_system::ActorSystem;
use aether_core::actor::context::{
    ContextHandle, MessagePart, SenderPart, SpawnerPart, StopperPart,
};
use aether_core::actor::message::{Message, MessageHandle};
...
async fn main() {
    ...
    let props =
        Props::from_sync_actor_producer(move |_| HelloActor::new(cloned_wait_group.clone())).await;
