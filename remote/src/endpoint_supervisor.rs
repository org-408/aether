...
use aether_core::actor::actor::{
    Actor, ActorError, ErrorReason, ExtendedPid, Props, RestartStatistics,
};
use aether_core::actor::actor_system::ActorSystem;
use aether_core::actor::context::{BasePart, ContextHandle, MessagePart, SpawnerPart};
use aether_core::actor::dispatch::{MailboxHandle, MailboxProducer};
    ...
    pub async fn spawn_endpoint_writer(
        ...
        let props = Props::from_async_actor_producer_with_opts(
            ...
            [Props::with_mailbox_producer(
                self.endpoint_writer_mailbox_producer(),
            )],
        )
        .await;
...
impl Actor for EndpointSupervisor {
    async fn receive(&mut self, context_handle: ContextHandle) -> Result<(), ActorError> {
        ...
        let address_opt = context_handle
            .get_message_handle()
            .await
            .to_typed::<String>();
        ...
        let e = Endpoint::new(
            endpoint_writer_pid.inner_pid,
            endpoint_watcher_pid.inner_pid,
        );
