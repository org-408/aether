...
use crate::generated::remote::{
    ConnectRequest, ConnectResponse, MessageBatch, MessageEnvelope, MessageHeader, RemoteMessage,
    ServerConnection,
};
use crate::messages::{
    EndpointConnectedEvent, EndpointEvent, EndpointTerminatedEvent, RemoteDeliver,
};
...
use aether_core::actor::actor::{Actor, ActorError, ErrorReason, ExtendedPid};
use aether_core::actor::actor_system::ActorSystem;
use aether_core::actor::context::{
    BasePart, ContextHandle, InfoPart, MessagePart, SenderPart, StopperPart,
};
use aether_core::actor::dispatch::DeadLetterEvent;
use aether_core::actor::message::{Message, MessageHandle, ReadonlyMessageHeaders};
use aether_core::generated::actor::{DeadLetterResponse, Pid};
...
impl EndpointWriter {
    ...
    async fn create_channel(&self) -> Result<Channel, EndpointWriterError> {
        ...
        let endpoint = Channel::from_shared(address)
            .map_err(|e| EndpointWriterError::InvalidUrl(e.to_string()))?;
        endpoint
            .connect()
    ...
    fn get_connect_response(
        remote_message: RemoteMessage,
    ) -> Result<ConnectResponse, EndpointWriterError> {
        match &remote_message.message_type {
            ...
            Some(message_type) => match message_type {
                ...
                _ => {
                    ...
                    Err(EndpointWriterError::Connection(
        ...
        for msg in msg_list {
            ...
            if self.get_stream().await.is_none() {
                ...
                if let Some(sender) = rd.sender {
                    ...
                    self.publish_stream(MessageHandle::new(DeadLetterEvent {
            ...
            let type_id = add_to_lookup(
                &mut type_names,
                message.get_type_name(),
                &mut type_names_arr,
            );
            let target_id =
                add_to_target_lookup(&mut target_names, &rd.target, &mut target_names_arr);
            let target_request_id = rd.target.request_id;

            let sender_id =
                add_to_sender_lookup(&mut sender_names, rd.sender.as_ref(), &mut sender_names_arr);
...
fn add_to_sender_lookup(
    m: &mut DashMap<String, i32>,
    pid: Option<&Pid>,
    arr: &mut Vec<Pid>,
) -> i32 {
    let pid = match pid {
