...
use crate::messages::{
    EndpointEvent, EndpointTerminatedEvent, Ping, Pong, RemoteDeliver, RemoteTerminate,
    RemoteUnwatch, RemoteWatch,
};
use crate::remote::Remote;
use aether_core::actor::actor::{ExtendedPid, Props, SpawnError};
use aether_core::actor::actor_system::ActorSystem;
use aether_core::actor::context::{SenderPart, SpawnerPart, StopperPart};
use aether_core::actor::dispatch::future::ActorFutureError;
use aether_core::actor::dispatch::DeadLetterEvent;
use aether_core::actor::message::MessageHandle;
use aether_core::actor::supervisor::{RestartingStrategy, SupervisorStrategyHandle};
use aether_core::event_stream::{EventHandler, Predicate, Subscription};
use aether_core::generated::actor::Pid;
use aether_utils::collections::DashMapExtension;
use dashmap::DashMap;
use std::fmt::Debug;
...
impl EndpointManager {
    ...
    pub async fn start(&mut self) -> Result<(), EndpointManagerError> {
        ...
        self.set_endpoint_subscription(
            event_stream
    ...
    async fn waiting(&self, duration: Duration) -> Result<(), EndpointManagerError> {
        ...
        let f = root
            .request_future(pid, MessageHandle::new(Ping), duration)
            .await;
        let msg = f
            .result()
            .await
