use aether_core::actor::actor::ExtendedPid;
use aether_core::actor::actor_system::ActorSystem;
use aether_core::actor::context::SenderPart;
use aether_core::actor::message::{
    MessageEnvelope, MessageHandle, MessageHeaders, SystemMessage,
};
use aether_core::actor::process::Process;
use aether_core::generated::actor::{Pid, Stop, Terminated, Unwatch, Watch};
...
use crate::generated::remote::{
    ConnectRequest, GetProcessDiagnosticsRequest, GetProcessDiagnosticsResponse,
    ListProcessesRequest, ListProcessesResponse, MessageBatch, RemoteMessage, ServerConnection,
};
...
impl EndpointReader {
    ...
    async fn on_server_connection(
            ...
            .await
        {
            ...
            {
                tracing::error!(
                    "EndpointReader failed to send ConnectResponse message: {}",
                    e
                );
            }
        } else {
    ...
    async fn on_message_batch(
        ...
        for envelope in &message_batch.envelopes {
            ...
            // TODO
            let serializer_id =
                SerializerId::try_from(envelope.serializer_id).expect("Invalid serializer id");

            let result = match deserialize_any(
                data,
                &serializer_id,
                "aether_remote::generated::cluster::PubSubBatchTransport",
            ) {
                Ok(v) => Some(v),
                Err(_) => match deserialize_any(
                    data,
                    &serializer_id,
                    "aether_remote::generated::cluster::DeliverBatchRequestTransport",
                ) {
                    Ok(v) => Some(v),
                    Err(_) => match deserialize_any(
                        data,
                        &serializer_id,
                        "aether_remote::generated::cluster::PubSubAutoRespondBatchTransport",
                    ) {
                        Ok(v) => Some(v),
                        Err(_) => None,
                    },
            ...
            match result {
                ...
                None => {
                    ...
                    if sender_opt.is_none() && envelope.message_header.is_none() {
                        tracing::info!(
                            "EndpointReader received message with no sender and no header"
                        );
                        self.get_actor_system()
                            .await
                            .get_root_context()
    ...
    pub fn set_suspend(&mut self, suspend: bool) {
        self.suspended
            .store(suspend, std::sync::atomic::Ordering::SeqCst);
    }
...
impl Remoting for EndpointReader {
    ...
    async fn receive(
        ...
        let endpoint_reader_connections = self
            .get_endpoint_manager()
            .await
            ...
            async move {
                if Self::get_disconnect_flg(cloned_disconnect_rx).await {
                    ...
                    if let Err(e) = cloned_response_tx
                        .send(Ok(RemoteMessage {
                            message_type: Some(
                                remote::remote_message::MessageType::DisconnectRequest(
                    ...
                    {
                        tracing::error!(
                            "EndpointReader failed to send disconnection message: {}",
                            e
                        );
                    }
                } else {
                        ...
                        Ok(remote_msg) => {
                            ...
                            match remote_msg.message_type {
                                Some(message_type) => match message_type {
                                    remote::remote_message::MessageType::ConnectRequest(
                                        connect_req,
                                    ) => {
                                        if let Err(e) = cloned_self
                                            .on_connect_request(&cloned_response_tx, &connect_req)
                                            .await
                                        {
                                            tracing::error!(
                                                "Failed to handle connect request, {}",
                                                e
                                            );
                                            break;
                                        }
                                    }
                                    remote::remote_message::MessageType::MessageBatch(
                                        message_batch,
                                    ) => {
                                        if let Err(e) =
                                            cloned_self.on_message_batch(&message_batch).await
                                        {
                                            tracing::error!(
                                                "Failed to handle message batch, {}",
                                                e
                                            );
                                            break;
                                        }
