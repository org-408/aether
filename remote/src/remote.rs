...
use aether_core::actor::message::{MessageHandle, ReadonlyMessageHeadersHandle};
use aether_core::actor::process::process_registry::AddressResolver;
use aether_core::actor::process::ProcessHandle;
use aether_core::extensions::{next_extension_id, Extension, ExtensionId};
...
impl Remote {
        ...
        Fut: Future<Output = ()> + Send + Sync,
    {
        ...
        process_registry
            ...
            .await;
        process_registry
            .set_address(format!("{}:{}", host_str, port))
            .await;
        ...
        if router
            .serve_with_shutdown(socket_addr, shutdown_future)
            .await
            .is_err()
        {
    ...
    fn configure_server(mut server: Server, sc: &ServerConfig) -> Server {
        ...
        server = server
            .http2_max_pending_accept_reset_streams(sc.http2_max_pending_accept_reset_streams);
...
#[cfg(test)]
mod tests {
    use aether_core::actor::actor::{Actor, ActorError, Props};
    use aether_core::actor::actor_system::ActorSystem;
    use aether_core::actor::context::{
        BasePart, ContextHandle, MessagePart, SenderPart, SpawnerPart,
    };
    use aether_core::actor::message::Message;
    use aether_core::actor::message::{MessageHandle, ResponseHandle};
    ...
    use aether_utils::concurrent::WaitGroup;
    ...
    impl Actor for EchoActor {
        async fn receive(&mut self, ctx: ContextHandle) -> Result<(), ActorError> {
            if let Some(msg) = ctx.get_message_handle().await.to_typed::<EchoMessage>() {
                ...
                ctx.respond(ResponseHandle::new(EchoMessage::new(format!(
                    "Echo: {}",
                    msg.message
                ))))
                .await;
    ...
    async fn test_remote_communication() {
        let _ = env::set_var("RUST_LOG", "aether_core=info");
        let _ = tracing_subscriber::fmt()
            .with_env_filter(EnvFilter::from_default_env())
            .try_init();
