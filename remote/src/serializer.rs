...
impl<T: Message + ProstMessage + Default + 'static> SerializerAny for ProtoSerializer<T> {
    fn serialize_any(&self, msg: &dyn Any) -> Result<Vec<u8>, SerializerError> {
        msg.downcast_ref::<T>()
            .ok_or(SerializerError::SerializationError(
...
impl<T: Message + Serialize + for<'de> Deserialize<'de> + Send + Sync + 'static> SerializerAny
    for JsonSerializer<T>
{
    fn serialize_any(&self, msg: &dyn Any) -> Result<Vec<u8>, SerializerError> {
        msg.downcast_ref::<T>()
            .ok_or(SerializerError::SerializationError(
...
static SERIALIZERS: Lazy<DashMap<SerializerKey, Arc<dyn Any + Send + Sync>>> =
...
pub fn find_serializer<T: 'static>(
    serializer_id: &SerializerId,
    type_name: &str,
...
pub fn find_serializer_any(
    serializer_id: &SerializerId,
    type_name: &str,
) -> Option<Arc<dyn SerializerAny>> {
...
pub fn serialize<T: 'static>(
    ...
    let serializer = find_serializer::<T>(serializer_id, std::any::type_name::<T>())
...
pub fn deserialize_any(
    ...
    let serializer =
        find_serializer_any(serializer_id, type_name).ok_or(SerializerError::UnknownType)?;
...
pub fn initialize_json_serializers<
...
pub fn initialize_proto_serializers<T: Message + Default + ProstMessage + Send + Sync + 'static>(
    ...
    register_serializer(
        SerializerId::Proto,
        Arc::new(ProtoSerializer::<T>::default()),
    )?;
    register_serializer_any(
        SerializerId::Proto,
        Arc::new(ProtoSerializer::<T>::default()),
    )?;
    Ok(())
