...
use dashmap::DashMap;
use aether_core::actor::actor::{
    Actor, ActorError, ErrorReason, ExtendedPid, Props, SpawnError,
};
use aether_core::actor::actor_system::ActorSystem;
use aether_core::actor::context::{
    BasePart, ContextHandle, InfoPart, MessagePart, SenderPart, SpawnerPart,
};
use aether_core::actor::dispatch::future::{ActorFuture, ActorFutureError};
        ...
        if let Some(msg) = message_handle.to_typed::<ActorPidRequest>() {
            return match self.get_kinds().get(&msg.kind) {
                ...
                Some(props) => {
                    ...
                    match ctx
                        .spawn_named(props.clone(), &format!("remote-{}", &name))
                        .await
                    {
                        Ok(pid) => {
                            context_handle
                        ...
                        Err(spawn_error) => match spawn_error {
                            SpawnError::ErrNameExists(pid) => {
                                context_handle
                                    .respond(ResponseHandle::new(ActorPidResponse {
                                        pid: Some(pid.inner_pid),
                                        status_code: ResponseStatusCode::ProcessNameAlreadyExists
                                            as i32,
                                    }))
                                    .await;
                                Err(ActorError::ReceiveError(ErrorReason::new(
