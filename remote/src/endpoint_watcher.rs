...
use dashmap::DashMap;
use aether_core::actor::actor::{Actor, ActorError, ExtendedPid, PidSet};
use aether_core::actor::actor_system::ActorSystem;
use aether_core::actor::context::{ContextHandle, InfoPart, MessagePart, StopperPart};
use aether_core::actor::message::{MessageHandle, SystemMessage};
use aether_core::actor::process::Process;
use aether_core::generated::actor::{Terminated, TerminatedReason, Unwatch, Watch};
    ...
    async fn connected(&mut self, mut ctx: ContextHandle) -> Result<(), ActorError> {
        ...
        if let Some(remote_terminate) = msg.to_typed::<RemoteTerminate>() {
            ...
            if let Some(mut entry) = self.watched.get_mut(&watcher_id) {
                ...
                if let Some(ref_process) = system
                    .get_process_registry()
                    .await
                    .get_local_process(&watcher_id)
                    .await
                {
                    ...
                    ref_process
                        .send_system_message(
                            &pid,
                            MessageHandle::new(SystemMessage::Terminate(msg)),
                        )
                        .await;
                }
            }
