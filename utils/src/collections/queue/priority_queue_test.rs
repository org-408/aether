...
mod tests {
    ...
    use crate::collections::{
        MpscUnboundedChannelQueue, PriorityMessage, PriorityQueue, QueueBase, QueueReader,
        QueueSize, QueueWriter,
    };
    use std::fmt::Debug;
    use std::sync::Arc;
    ...
    async fn test_push_pop_ring_2() {
        let mut q: PriorityQueue<
            TestMessageBaseHandle,
            MpscUnboundedChannelQueue<TestMessageBaseHandle>,
        > = new_priority_ring_queue().await;

        for _ in 0..2 {
            let msg =
                TestMessageBaseHandle::new(TestPriorityMessage::new("7 hello".to_string(), 7));
            q.offer(msg.clone()).await.unwrap();
        }

        for _ in 0..2 {
            let msg =
                TestMessageBaseHandle::new(TestPriorityMessage::new("5 hello".to_string(), 5));
            q.offer(msg.clone()).await.unwrap();
        }

        for _ in 0..2 {
            let msg =
                TestMessageBaseHandle::new(TestPriorityMessage::new("0 hello".to_string(), 0));
            q.offer(msg.clone()).await.unwrap();
        }

        for _ in 0..2 {
            let msg =
                TestMessageBaseHandle::new(TestPriorityMessage::new("6 hello".to_string(), 6));
            q.offer(msg.clone()).await.unwrap();
        }
